...
Node* rotateLL(Node* node) {
	...
	setHeight(node); //회전 이후에 높이 다시 계산
	return leftChild; //leftChild 는 이후 setHeight()수행
}
...
Node* rotateLR(Node* node) {
	...
	setHeight(node->leftChild); //회전 이후, 높이 다시 계산
...
Node* rotateRL(Node* node) {
	...
	setHeight(node->rightChild); //회전 이후, 높이 다시 계산
